...
describe('cache', () => {
  ...
  it('cache latest versions', async () => {
    ...
    await ncu({ packageData, cache: true, cacheFile, peer: true })

    const cacheData: CacheData = await fs.readFile(cacheFile, 'utf-8').then(JSON.parse)
  ...
  it('use different cache key for different target', async () => {
    ...
    const cacheData1: CacheData = await fs.readFile(cacheFile, 'utf-8').then(JSON.parse)
    ...
    const cacheData2: CacheData = await fs.readFile(cacheFile, 'utf-8').then(JSON.parse)
  ...
  it('expires cache when schema version does not match', async () => {
    ...
    const newCache = await fs.readFile(cacheFile, 'utf-8').then(JSON.parse)
  ...
  it('expires cache when timestamp is older than 10 minutes', async () => {
    ...
    const cacheData = await fs.readFile(cacheFile, 'utf-8').then(JSON.parse)
